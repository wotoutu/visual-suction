...
class Logger():
    ...
    def __init__(self, continue_logging, logging_directory):
        ...
        if self.continue_logging:
            ...
            self.base_directory = os.path.join(logging_directory, timestamp_value.strftime('%Y-%m-%d_%H-%M-%S'))#%Y-%m-%d.%H:%M:%S
    ...
    def save_camera_info(self, intrinsics, pose, depth_scale):
        np.savetxt(os.path.join(self.info_directory, 'camera-intrinsics.txt'), intrinsics, delimiter=' ')#保存相机内参矩阵
        np.savetxt(os.path.join(self.info_directory, 'camera-pose.txt'), pose, delimiter=' ')#保存相机外参矩阵
        np.savetxt(os.path.join(self.info_directory, 'camera-depth-scale.txt'), [depth_scale], delimiter=' ')#保存深度缩放系数

    def save_heightmap_info(self, boundaries, resolution):
        np.savetxt(os.path.join(self.info_directory, 'heightmap-boundaries.txt'), boundaries, delimiter=' ')#3x2矩阵，定义机器人工作空间范围
        np.savetxt(os.path.join(self.info_directory, 'heightmap-resolution.txt'), [resolution], delimiter=' ')#高度图分辨率（米/像素）

    def save_images(self, iteration, color_image, depth_image, mode):
        color_image = cv2.cvtColor(color_image, cv2.COLOR_RGB2BGR)#原始RGB-D图像存储
        cv2.imwrite(os.path.join(self.color_images_directory, '%06d.%s.color.png' % (iteration, mode)), color_image)
        depth_image = np.round(depth_image * 10000).astype(np.uint16) #深度图存储为16位PNG（1像素=0.1毫米）
    ...
    def save_transition(self, iteration, transition):#存储动作执行前后环境的深度图变化
